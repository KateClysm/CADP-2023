...
Begin
	...
	while (listaP <> nil) do	//mientras que la lista que estmeos analizando no esté vacía
	begin
		ISBNactual:=listaP^.dato.ISBN;		//el ISBN actual es igual al del nodo que analizamos
		registroActual.ISBN:= ISBNactual;	//se crea el registro con el ISBN
		registroActual.cant:=0;				//y la cantidad de veces que fue prestado en 0
		
		while (listaP <> nil) AND (listaP^.dato.ISBN = ISBNactual) do	// si el ISBN sigue siendo el mismo
		begin
			registroActual.cant:= registroActual.cant + 1; 	//sumamos uno a la cantidad del registro
			...
			listaP := listaP^.sig;	//cambiamos de nodo a analizar
		end;
		
		//si se salió del while anterior es porque el ISBN cambió
		
		insertarOrdenado(listaVP, registroActual); //insertamos de manera ordenada en nuestra lista de veces prestado el registro Actual con su contador completo.
		
		ISBNactual:=listaP^.dato.ISBN;	//asi que cambiamos nuestro ISBN actual y repetimos el proceso
	end;
